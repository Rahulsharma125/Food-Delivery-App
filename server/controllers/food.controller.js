...
async function createFood(req, res){
    const { name, price, description, rating, restaurant } = req.body;
    if(!name || !price || !description || !rating || !restaurant){
...
async function updateFood(req, res){
    const { name, price, description, rating, restaurant} = req.body;
    const { id } = req.params;
    if( !name || !price || !description || !rating || !restaurant || !id){
        return res.status(400).send("All fields are required!");
    }
    ...
    const updatedFood = await Food.findByIdAndUpdate(
        id,
        {
            name, price, description, rating, restaurant,
        }, 
        {
            new: true,
        }
